...
// initial copy of RunDrtExample in org.matsim.codeexamples.extensions.dvrp
    ...
    public static void run(boolean otfvis, String... args ){
        ...
        }
        {
            ...
            config.strategy().addStrategySettings( new ReplanningConfigGroup.StrategySettings().setStrategyName( DefaultPlanStrategiesModule.DefaultStrategy.ChangeSingleTripMode ).setWeight( 0.1 ) );
            config.changeMode().setModes( new String[]{ TransportMode.car, DRT_A, DRT_B, DRT_C} );

            // have a "normal" plans choice strategy:
            config.strategy().addStrategySettings( new ReplanningConfigGroup.StrategySettings().setStrategyName( DefaultPlanStrategiesModule.DefaultSelector.ChangeExpBeta ).setWeight( 1. ) );
        }
        {
            // add params so that scoring works:
            config.planCalcScore().addModeParams( new ScoringConfigGroup.ModeParams( DRT_A ) );
            config.planCalcScore().addModeParams( new ScoringConfigGroup.ModeParams( DRT_B ) );
            config.planCalcScore().addModeParams( new ScoringConfigGroup.ModeParams( DRT_C ) );
        }
